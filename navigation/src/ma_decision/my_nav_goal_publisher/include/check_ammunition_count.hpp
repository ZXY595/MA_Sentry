...
public:
      ...
      : BT::ConditionNode(name, config)
  {
    ...
    node_->declare_parameter<int>("ammunition_count", 400);  // 初始化弹丸数

    t_start = node_->now().seconds();

    // 创建订阅器来接收弹丸数量
    ammunition_subscriber_ = node_->create_subscription<rm_interfaces::msg::SerialReceiveData>(
  ...
  BT::NodeStatus tick() override
  {
    ...
    // 判断弹丸数量是否低于阈值
    if (ammunition_count_ < threshold || blood_count_  < 80)
    {
